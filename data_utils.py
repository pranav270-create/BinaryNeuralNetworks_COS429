...
mnist_dset_train = torchvision.datasets.MNIST(
    './mnist_pytorch', train=True, transform=transform, target_transform=None, download=True)
mnist_train_loader = torch.utils.data.DataLoader(
    mnist_dset_train, batch_size=100, shuffle=True, num_workers=1)
...
mnist_test_loader = torch.utils.data.DataLoader(
    mnist_dset_test, batch_size=100, shuffle=False, num_workers=1)

fmnist_dset_train = torchvision.datasets.FashionMNIST(
    './fmnist_pytorch', train=True, transform=transform, target_transform=None, download=True)
fashion_mnist_train_loader = torch.utils.data.DataLoader(
    fmnist_dset_train, batch_size=100, shuffle=True, num_workers=1)

fmnist_dset_test = torchvision.datasets.FashionMNIST(
    './fmnist_pytorch', train=False, transform=transform, target_transform=None, download=True)
fashion_mnist_test_loader = torch.utils.data.DataLoader(
    fmnist_dset_test, batch_size=100, shuffle=False, num_workers=1)
...
def create_permuted_loaders(task):
    ...
    transform = torchvision.transforms.Compose([torchvision.transforms.ToTensor(),
                                                torchvision.transforms.Lambda(
                                                    lambda x: x.view(-1)[permut].view(1, 28, 28)),
    ...
    if task == 'MNIST':
        ...
        train_loader = torch.utils.data.DataLoader(
            dset_train, batch_size=100, shuffle=True, num_workers=1)
...
def process_features(X_train, X_test, mode):
    if mode == "cutoff":
        ...
        X_train = (X_train - X_train.mean(axis=0, keepdims=True)) / \
            X_train.var(axis=0, keepdims=True)  # ???
        X_test = (X_test - X_test.mean(axis=0, keepdims=True)) / \
            X_test.var(axis=0, keepdims=True)
        ...
        X_train = (X_train - X_train.mean(axis=1, keepdims=True)) / \
            X_train.var(axis=1, keepdims=True)  # Instance norm
        X_test = (X_test - X_test.mean(axis=1, keepdims=True)) / \
            X_test.var(axis=1, keepdims=True)
        ...
        X_train = (X_train - X_train.mean(keepdims=True)) / \
            X_train.var(keepdims=True)  # Batch norm
        X_test = (X_test - X_test.mean(keepdims=True)) / \
            X_test.var(keepdims=True)
    elif mode == "rescale":
...
def process_cifar10(subset):
    ...
    cifar_X_train = torch.load(
        'cifar10_features_dataset/train.pt').cpu().numpy()
    cifar_Y_train = torch.load(
        'cifar10_features_dataset/train_targets.pt').cpu().numpy()
    cifar_X_test = torch.load('cifar10_features_dataset/test.pt').cpu().numpy()
    cifar_Y_test = torch.load(
        'cifar10_features_dataset/test_targets.pt').cpu().numpy()
    ...
    sub_train_loader = torch.utils.data.DataLoader(
        sub_dset_train, batch_size=100, shuffle=True, num_workers=4)

    sub_dset_test = DatasetProcessing(sub_X_test, sub_Y_test)
    sub_test_loader = torch.utils.data.DataLoader(
        sub_dset_test, batch_size=100, shuffle=False, num_workers=0)
...
def process_cifar100(n_subset):
    ...
    cifar100_X_train = torch.load(
        'cifar100_features_dataset/train.pt').cpu().numpy()
    cifar100_Y_train = torch.load(
        'cifar100_features_dataset/train_targets.pt').cpu().numpy()
    cifar100_X_test = torch.load(
        'cifar100_features_dataset/test.pt').cpu().numpy()
    cifar100_Y_test = torch.load(
        'cifar100_features_dataset/test_targets.pt').cpu().numpy()

    for k in range(n_subset):
        partition = np.vectorize(lambda l: (
            (l < (k+1)*subset_size) and (l >= k*subset_size)))
        mode = 'mean_over_pixels'
        sub_X_train = cifar100_X_train[partition(cifar100_Y_train)]
        sub_X_test = cifar100_X_test[partition(cifar100_Y_test)]

        sub_X_train, sub_X_test = process_features(
            sub_X_train, sub_X_test, mode)
        ...
        sub_dset_train = DatasetProcessing(sub_X_train, sub_Y_train)
        sub_train_loader = torch.utils.data.DataLoader(
            sub_dset_train, batch_size=20, shuffle=True, num_workers=4)

        sub_dset_test = DatasetProcessing(sub_X_test, sub_Y_test)
        sub_test_loader = torch.utils.data.DataLoader(
            sub_dset_test, batch_size=20, shuffle=False, num_workers=0)
